...
SERVER_NAME = "LAPTOP-MF2AAKP7\SQLEXPRESS"
...
class HomePage(tk.Frame):
    def __init__(self, parent, Controller):
        ...
        cal = DateEntry(frame2,width=15, foreground="grey", date_pattern='dd/mm/yyyy')
        cal.grid(row=1, column=2)
        """ cal = DateEntry(frame2, selectmode = 'day', year=2022)
        cal.grid(column=2, row=1) """ 
        self.label_notice2 = tk.Label(frame2, text="")
        self.label_notice2.grid(column=3, row=1)

        search = tk.Button(frame2, text="Search", bg='cornflowerblue', command=lambda: self.drawTree(parent, Controller, Combo))
    ...
    def drawTree(self, parent, appController, Combo):
        tk.Frame.__init__(self, parent)
